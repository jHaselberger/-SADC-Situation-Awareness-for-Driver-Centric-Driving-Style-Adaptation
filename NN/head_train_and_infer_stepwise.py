...
if __name__ == '__main__':
    ...
    _, pretrain_labels, _, pretrain_representations = load_dataset(
        kpi_name=config["filtering"]["kpi_name"],
        driving_data_path=config["dataset"]["pretrain_driving_data_path"],
        representation_file_path = config["dataset"]["pretrain_representation_file_path"],
    ...
    max_norm_value = pretrain_labels.max()

    scaler = StandardScaler()
    scaler.fit(pretrain_representations)

    df_val_train_all = []
    df_val_val_all = []
    for alias in config["filtering"]["aliases"]:    
        iterative_training_splits_df = pd.read_pickle(config["dataset"]["val_train_training_splits_path"])
        checkpoint_path = None

        df_val_train_predict, _, _, val_train_representations_predict = load_dataset(
                kpi_name=config["filtering"]["kpi_name"],
        ...
        val_train_representations_predict = scaler.transform(val_train_representations_predict)
        
        df_val_val_predict, _, _, val_val_representations_predict = load_dataset(
                kpi_name=config["filtering"]["kpi_name"],
        ...
        val_val_representations_predict = scaler.transform(val_val_representations_predict)

        for train_iter in iterative_training_splits_df["train_iter"].unique():
        
            test, val_train_labels_it, _, val_train_representations_it = load_dataset(
                kpi_name=config["filtering"]["kpi_name"],
                driving_data_path=config["dataset"]["val_train_driving_data_path"],
                representation_file_path = config["dataset"]["val_train_representation_file_path"],
                filter_nans=config["filtering"]["filter_nans"],
                ...
                filter_stationary=config["filtering"]["filter_stationary"],
                load_representations=True,
                iterative_training_splits_df=iterative_training_splits_df,
            ...
            val_train_labels_it = torch.unsqueeze(torch.Tensor(normalize(val_train_labels_it, max_norm_value)), dim=1)
            val_val_labels = torch.unsqueeze(torch.Tensor(normalize(val_val_labels, max_norm_value)), dim=1)

            val_train_representations_it = scaler.transform(val_train_representations_it)
            val_val_representations = scaler.transform(val_val_representations)
           
            print(f"val_train_it label max after normalization: {val_train_labels_it.max()}")
            print(f"val_train_it label min after normalization: {val_train_labels_it.min()}")
            print(f"val_train_it label mean after normalization: {val_train_labels_it.mean()}")
            print(f"val_train_it label shape: {val_train_labels_it.shape}")
            print(f"val_train_it representations len: {len(val_train_representations_it)}")
            ...
            model_suit = HeadModelSuit( 
                        ...
                        "use_wandb":config["logging"]["use_wandb"],
                            ...
                            "name": config["encoder_name"] + "_" + config["head_name"] + "_" + alias + "_stepwise_" + str(train_iter)},
                        train_labels=val_train_labels_it,
                        val_labels=val_val_labels,
                        train_representations=val_train_representations_it, 
                        ...
                        target_alias=f"{alias}_stepwise",
                        from_checkpoint_path=None,
                        use_early_stopping=True)

            if checkpoint_path:
                print(f"loading model weights from checkpoint: {checkpoint_path}")
                model_suit.load_model_weights(checkpoint_path)
            model_suit.train()

            wandb.finish()
         
            checkpoint_path = os.path.join(config["export"]["output_dir"], config["encoder_name"], "heads", config["head_name"], f"{alias}_stepwise", "checkpoints", config["validation"]["checkpoint_name"])
            ...
            model_suit = HeadModelSuit( 
                ...
                "use_wandb":config["logging"]["use_wandb"],
                    "project_name":config["logging"]["wandb_project_name"], 
                    "save_dir":config["logging"]["wandb_save_dir"],
                    "name": config["encoder_name"] + "_" + config["head_name"] + "_" + alias +  "_stepwise_" + str(train_iter) + "_infer"},
                train_labels=val_train_labels_it,
                val_labels=val_val_labels,
                train_representations=val_train_representations_it, 
                val_representations=val_val_representations,
            ...
            val_train_predictions = model_suit.predict(val_train_representations_predict)
            df_val_train_predict[f"predictions_{train_iter}"] = copy.deepcopy(val_train_predictions)
           
            val_val_predictions = model_suit.predict(val_val_representations_predict)
            df_val_val_predict[f"predictions_{train_iter}"] = copy.deepcopy(val_val_predictions)

            wandb.finish()

        df_val_train_all.append(copy.deepcopy(df_val_train_predict))
        df_val_val_all.append(copy.deepcopy(df_val_val_predict))
            
    output_dir = os.path.join(config["export"]["output_dir"], config["encoder_name"], "heads", config["head_name"], "predictions_stepwise", "val_train")
    ...
    output_dir = os.path.join(config["export"]["output_dir"], config["encoder_name"], "heads", config["head_name"], "predictions_stepwise", "val_val")
    os.makedirs(output_dir, exist_ok=True)
